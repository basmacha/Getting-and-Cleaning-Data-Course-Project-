...
tidy_data<-data.frame((HumanData[grep("[Mm]ean|[Ss]td", colnames(HumanData), value=TRUE)]), "subject"=HumanData$subject, "activity"=HumanData$activity, "activity label"=HumanData$`activity label`)
...
complete_set<-tidy_data %>% group_by(subject, activity.label) %>% summarize_all(funs(mean))
